...
def edge_lists_intersection(edges1, edges2):
    set1 = set(tuple(edge) for edge in edges1)
    set2 = set(tuple(edge) for edge in edges2)
...
#     low, high = 4, min(50, n_cells - 1)
#     combined_omics = np.hstack(omics)

#     for _ in range(max_iter):
#         n_neighbors = (low + high) // 2
#         edge_index_spatial = get_single_edge_index(spatial, n_neighbors)
#         edge_index_feat = get_expr_edge_index(combined_omics, n_neighbors)
...
#     return torch.LongTensor(edge_index_intersect).T
...
def get_consensus_edges(spatial, 
    ...
    edge_index = torch.LongTensor(edge_index_intersect).T
    return edge_index
